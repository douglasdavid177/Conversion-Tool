...
const AboutSection = ({ setOpenNav }) => {
  ...
  return (
    <div className={styles.aboutSection}>
      ...
      <p>
        The{" "}
        <Link href="/decfracsection" className={styles.chooseToolButton}>
          decimal to fraction converter
        </Link>
        {` finds a fraction (with a given denominator) that's closest to or equivalent to a given decimal. The results page shows the closest and second closet fractions, allowing a user to see a high/low value pair corresponding to fractions above and below the target value. The results are color coded— showing values in yellow if they are below the target value, green if they are eqivalent, or blue if above. `}
      </p>
      <p>
        The{" "}
        <Link href="/baseconversionsection" className={styles.chooseToolButton}>
          number base conversion tool
        </Link>
        {` converts any numerical value between number systems with different base values. On the results screen, the larger base is displayed in blue while the smaller base is displayed in yellow. Equivalent values will appear green.`}
      </p>
      <p>
        The{" "}
        <Link href="/unitconversionsection" className={styles.chooseToolButton}>
          The unit conversion tool
        </Link>
        {` for measurements calculates equivalent measurements to a given value, using a vast list of ${amountOfUnits} different physical units across ${amountOfMeasurementTypes} domains of measurement. Color coding is used in the results section to suggest that a particular unit within the resulting pair is larger than the other if it is blue, or smaller if yellow. If the two units are the same, they will both appear green.`}
      </p>
      <p>
        The{" "}
        <Link
        ...
        {` (coming soon) converts any amount of currency from one type to an equivalent amount in another, using live exchange rate values that are updated every five minutes. Requires an internet connection. `}
      </p>
